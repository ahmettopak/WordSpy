        ...
        if (currentGameData != null) {
            ...
            foreach (var squares in currentGameData.selectedBoardData.Board) {
                foreach (var squareLetter in squares.Row) {
                    var normalLetterData = alphabetData.AlphabetNormal.Find(match: data => data.letter == squareLetter);
                    var selectedLetterData = alphabetData.AlphabetHighlighted.Find(match: data => data.letter == squareLetter);
                    var correctLetterData = alphabetData.AlphabetWrong.Find(match: data => data.letter == squareLetter);
